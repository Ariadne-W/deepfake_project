...
def main():
    ...
    folds = [[0], [1], [2], [3], [4]]
    print(folds)
    with open("/home/media/lzy/code/Celeb-DF/Celeb-DF-v2/List_of_testing_videos.txt") as f:
        lines = f.readlines()
    lines = [line.strip("\n").split(" ")[1].split("/")[-1] for line in lines]
    lines2 = [line[:-8]+'5'+line[-7:] for line in lines]
    lines = set(lines + lines2)
    print(lines)#test set
    video_fold = {}
    ...
    for img_path, label, ori_vid in data:
        ...
        assert video_fold[video] == video_fold[ori_vid], "original video and fake have leak  {} {}".format(ori_vid,
                                                                                                           video)
        if video+".mp4" in lines:
            print(video+".mp4 in test set, continue")
            continue
